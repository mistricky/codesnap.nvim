...
local function get_plugin_version()
  ...
  local project_toml_path = path_utils.join("/", vim.fn.fnamemodify(debug.getinfo(1).source:sub(2), ":p:h:h:h"), "project.toml")
...
local function download_lib(version, lib_name, dest_path)
  local url = string.format("https://github.com/mistricky/codesnap.nvim/releases/download/%s/%s", version, lib_name)
  
  -- Use curl to download the library
  local cmd = string.format("curl -L -o %s %s", vim.fn.shellescape(dest_path), vim.fn.shellescape(url))
...
function fetch.ensure_lib()
  ...
  local lib_dir = path_utils.join("/", vim.fn.fnamemodify(debug.getinfo(1).source:sub(2), ":p:h:h"), "libs")
  local lib_path = path_utils.join("/", lib_dir, lib_name)
  local version_file = path_utils.join("/", lib_dir, ".version")
